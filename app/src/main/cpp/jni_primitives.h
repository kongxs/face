...
public:
    JNI_VisionDetRet(JNIEnv *env) {
        ...
        jSetPoint = env->GetMethodID(detRetClass,"addPoint","(II)V");
    }
    ...
    void addPoint(JNIEnv *env ,jobject &jDetRet, const int x , const int y) {
        env->CallVoidMethod(jDetRet,jSetPoint,x,y);
    }
